...
def active_learning(train_df, validation_df, test_df):
    ...
    if config["timeout_predictor_usage"] == "Yes":
        ...
        if config["voting"] == "Hard":
            initial_val_runtime = round(hard_voting_mechanism(validation_df, active_algorithm_predictor_dict_filtered, active_learning_algorithms, config["max_timeout"], config["penalty"], active_timeout_predictor_dict) + train_df.loc[instance_cost_table["instance_id"].unique(), feature_costs_columns].sum().sum(), 2)
            initial_test_runtime = round(hard_voting_mechanism(test_df, active_algorithm_predictor_dict_filtered, active_learning_algorithms, config["max_timeout"], config["penalty"], active_timeout_predictor_dict) + train_df.loc[instance_cost_table["instance_id"].unique(), feature_costs_columns].sum().sum(), 2)
    else:
        if config["voting"] == "Hard":
            initial_val_runtime = round(hard_voting_mechanism(validation_df, active_algorithm_predictor_dict_filtered, active_learning_algorithms, config["max_timeout"], config["penalty"]) + train_df.loc[instance_cost_table["instance_id"].unique(), feature_costs_columns].sum().sum(), 2)
            initial_test_runtime = round(hard_voting_mechanism(test_df, active_algorithm_predictor_dict_filtered, active_learning_algorithms, config["max_timeout"], config["penalty"]) + train_df.loc[instance_cost_table["instance_id"].unique(), feature_costs_columns].sum().sum(), 2)

    initial_instance_cost = round(instance_cost_table["Runtime"].sum() , 2)

    logger.info(f"Initial Active Learning Random Query Runtime on Validation Set: {round(initial_val_runtime, 2)}")
    logger.info(f"Initial Active Learning Random Query Runtime on Test Set: {round(initial_test_runtime, 2)}")
    logger.info(f"Initial Active Learning Random Query Instance Cost: {round(initial_instance_cost, 2)}")
    ...
    bin_size = math.floor(len(pool_df) * len(active_algo_pair_comb_list) * config["query_size"] / 100)
    ...
    while(not all(active_algo_rf.get_pool_data().empty for active_algo_rf in active_algorithm_predictor_dict.values())):
        ...
        filtered_table = uncertainty_table.sample(frac= 1 ,random_state=round(1000000 * random.random())).head(bin_size)

        if config["timeout_predictor_usage"] == "Yes":
            filtered_table = uncertainty_table.sample(frac= 1 ,random_state=round(1000000 * random.random()))[~((uncertainty_table['Algorithm1_TO'] == 1) & (uncertainty_table['Algorithm2_TO'] == 1))].head(bin_size)
        ...
        if config["timeout_predictor_usage"] == "Yes":
            ...
            if config["voting"] == "Hard":
                query_val_runtime = round(hard_voting_mechanism(validation_df, active_algorithm_predictor_dict_filtered, active_learning_algorithms, config["max_timeout"], config["penalty"], active_timeout_predictor_dict) + train_df.loc[instance_cost_table["instance_id"].unique(), feature_costs_columns].sum().sum(), 2)
                query_test_runtime = round(hard_voting_mechanism(test_df, active_algorithm_predictor_dict_filtered, active_learning_algorithms, config["max_timeout"], config["penalty"], active_timeout_predictor_dict) + train_df.loc[instance_cost_table["instance_id"].unique(), feature_costs_columns].sum().sum(), 2)
        else:
            if config["voting"] == "Hard":
                query_val_runtime = round(hard_voting_mechanism(validation_df, active_algorithm_predictor_dict_filtered, active_learning_algorithms, config["max_timeout"], config["penalty"]) + train_df.loc[instance_cost_table["instance_id"].unique(), feature_costs_columns].sum().sum(), 2)
                query_test_runtime = round(hard_voting_mechanism(test_df, active_algorithm_predictor_dict_filtered, active_learning_algorithms, config["max_timeout"], config["penalty"]) + train_df.loc[instance_cost_table["instance_id"].unique(), feature_costs_columns].sum().sum(), 2)
        
        query_instance_cost = round(instance_cost_table["Runtime"].sum() , 2)

        logger.info(f"Query {i} Active Learning Random Query Runtime on Validation Set: {query_val_runtime}")
        logger.info(f"Query {i} Active Learning Random Query Runtime on Test Set: {query_test_runtime}")
        logger.info(f"Query {i} Active Learning Random Query Instance Cost: {query_instance_cost}")
...
def save_results():
    file_name_dict = {"runtime_train_score": runtime_train_dict,
                      "runtime_test_score": runtime_test_dict, 
...
logger.info("Feature cost of train set is %s", round(df_final.loc[train_index][feature_costs_columns].sum().sum(), 2))
...
instance_cost_dict["active_learning_random_query"] = active_instance_cost_history
runtime_test_dict["active_learning_random_query"] = active_runtime_test_history
runtime_validation_dict["active_learning_random_query"] = active_runtime_val_history
